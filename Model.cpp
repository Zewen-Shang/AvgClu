...
void Model::buildOct(int threshold)
{
	...
	splitCell(threshold,cellBuffer[0]);
}

void Model::splitCell(int threshold, OctCell& cell)
{
	if (cell.vertexs.size() > threshold) {
		OctCell sons[8];
		vector<BoundBox>sonBoxs = cell.box.getSons();
		for (int i = 0; i < 8; i++) {
			sons[i].id = cellBuffer.size() + i;
			sons[i].box = sonBoxs[i];
			cell.sons[i] = sons[i].id;
		}
		for (int vId : cell.vertexs) {
		...
		//cell.vertexs.clear();
		for (int i = 0; i < 8; i++) {
			splitCell(threshold, cellBuffer[sons[i].id]);
		}
	}
}
